...
This crate provides protobuf type definitions compatible with nockchain's gRPC API, along with conversion traits to/from `nbx-nockchain-types`.
...
```
nbx-nockchain-types (no_std, custom types)
          ↕ conversion layer
    nbx-grpc-proto (std, protobuf)
          ↕ tonic/gRPC
...
| nbx-nockchain-types | Protobuf | nockchain-types |
|---------------------|----------|-----------------|
| `nbx_ztd::Belt` | `Belt { value: u64 }` | `nockchain_math::Belt` |
| `nbx_ztd::Digest` ([Belt; 5]) | `Hash` (5 Belt fields) | `Hash` ([Belt; 5]) |
...
2. **Public Fields**: Made key fields public in nbx-nockchain-types for gRPC conversions:
   - `Name`: `first`, `last`
   - `Pkh`: `m`, `hashes`
   - `PkhSignature`: tuple field (Vec of PublicKey/Signature pairs)
   - `MerkleProof`: `root`, `path`
   - `Hax`: tuple field (Vec of Digest)
3. **Signature Conversion**: Converts between `nbx_crypto::Signature` (UBig c/s fields) and protobuf EightBelt arrays
...
```rust
use nbx_grpc_proto::client::{PublicNockchainGrpcClient, BalanceRequest};
...
- ✅ Complete type conversions between nbx-nockchain-types and protobuf
...
- [ ] Add reverse conversions (protobuf → nbx-nockchain-types) where needed
- [ ] Consider WASM compatibility for client-side gRPC-web

## Building

```bash
cargo build -p nbx-grpc-proto
```
